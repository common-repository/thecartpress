...
function tcp_get_the_title( $post_id = 0, $option_1_id = 0, $option_2_id = 0, $html = true, $show_parent = true ) {
	...
	$title = '';
	if ( $html ) {
		$title .= '<span class="tcp_nested_title">';
	}
	...
	if ( $option_1_id > 0 ) {
		...
		if ( $html ) {
			$title .= ' <span class="tcp_nested_option_1">';
		} else {
			$title .= ' - ';
		}
...
/**
 * Outputs the current product (post) title
 */
 ...
 * @since 1.0.9
 * @param $price
 * @paramn $currency, $currency_iso
 * @return formatting price with currency
 */
function tcp_format_the_price( $price, $currency = '') {
	...
	if ( strlen( $layout ) == 0 ) {
		// Uses default layout: 'currency + price + (currency ISO)'
		$layout = __( '%1$s%2$s (%3$s)', 'tcp' );
	}
 ...
 * 
 * @param $post_id, if 0 uses the current post id (wordpress loop)
 * @param $price, if has a value, uses this value to format
 * @since 1.1.1
 */
...
function tcp_get_the_shipping_tax() {
	...
	if ( $tax_id == 0 ) {
		return apply_filters( 'tcp_get_the_shipping_tax', 0 );
	}
	$country_iso = tcp_get_tax_country();
	$region_iso = tcp_get_tax_region();
	require_once( TCP_DAOS_FOLDER . 'TaxRates.class.php' );
...
function tcp_get_the_thumbnail_image( $post_id = 0, $args = false, $attr = false ) {
	if ( has_post_thumbnail( $post_id ) ) {
		...
		if ( function_exists( 'get_the_post_thumbnail' ) ) {
			$image = get_the_post_thumbnail( $post_id, $image_size, $attr );
		}
		return $image;
	}
}
 ...
 *
 * @since 1.1.8
 * @param $post_id, if 0 uses the current post id (worpdress loop)
 */
 ...
 * @since 1.1.8
 * @param $post_id, if 0 uses the current post id (worpdress loop)
 * @param $echo, if true outputs the result
 */
 ...
 * @since 1.1.8
 * @param $post_id, if 0 uses the current post id (worpdress loop)
 * @param $length, set exceprt length, if 0 uses the default one
 */
...
function tcp_get_product_types( $no_one = false, $no_one_desc = '' ) {
	$types = array();
	if ( $no_one ) {
		$types[''] = array( 'label' => $no_one_desc != '' ? $no_one_desc : __( 'No one', 'tcp' ) );
	}
	$types['SIMPLE'] = array(
		'label'	=> __( 'Simple', 'tcp' ),
	);
...
function tcp_update_premium_plugin( $plugin_file ) {
	...
	if ( class_exists( 'TCPAutoUpdate' ) ) {
		new TCPAutoUpdate( $plugin_file );
	}
}
