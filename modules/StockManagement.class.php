...
class TCPStockManagement {
	...
	function tcp_init( $thecartpress ) {
		...
		if ( $stock_management ) {
			...
			// Boths hooks for shopping cart lines
			add_action( 'tcp_cart_units'								, array( $this, 'tcp_cart_units' ), 10, 2 );
			...
			// Removes the unit fields if no units enough
			...
			add_filter( 'tcp_sort_main_loop'							, array( $this, 'tcp_sort_main_loop' ) );
...
<div id="tcp-stock-options" style="display: none;">
	...
	/**
	 * Adds “out of stock" in the shopping cart lines, if no units enough
	 *
	 * @param $item, each line in the shoppingCart
	 */
	...
	/**
	 * Allows to change the unit field in the shopping cart
	 * 
	 * @param $out
	 * @param $order_detail
	 */
	function tcp_shopping_cart_page_units( $out, $order_detail ) {
		$stock = tcp_get_the_stock( $order_detail->get_post_id() );
		//if ( $stock != -1 && $stock == $order_detail->get_qty_ordered() ) {
		if ( $stock == 1 ) {
			return $order_detail->get_qty_ordered();
		}
		return $out;
	}
...
*/ 
	...
	/**
	 * Removes the unit fields if no units enough, or hides the unit fields if only one unit is available
	 * 
	 * @param $out
	...
	function tcp_the_add_to_cart_unit_field( $out, $post_id ) {
		...
		if ( $stock == 0 ) {
			return '<span class="tcp_no_stock">' . __( 'Out of stock', 'tcp' ) . '</span>';
			
		// Since 1.4.3, if units in stock are less or equal to shopingcart, it hides the units fields
		} elseif ( $stock == 1 ) {
			return '<input type="hidden" name="tcp_count[]" id="tcp_count_' . $post_id . '" value="1" class="tcp_count form-control" />';
		} elseif ( $stock > 1 ) {
			$shopingcart = thecartpress()->getShoppingCart();
			$item = $shopingcart->getItem( $post_id );
			if ( $item !== false ) {
	...
	function tcp_sort_main_loop( $query ) {
		if ( thecartpress()->get_setting( 'hide_out_of_stock' ) ) {
