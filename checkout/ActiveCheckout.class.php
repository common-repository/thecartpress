	...
	static function sendOrderMails( $order_id, $additional_msg = '', $for_customer = true, $for_merchant = true ) {
		...
		if ( $order ) {
			...
			if ( $for_customer ) {
				...
				// Adds shipping email, if exists
				if ( strlen( $order->shipping_email ) > 0 ) {
					$customer_email[] = $order->shipping_email;
				}

				// Adds billinf email, if exists and it's different from shipping email
				if ( strlen( $order->billing_email ) > 0 && $order->shipping_email != $order->billing_email ) {
				...
				wp_mail( $to, $subject, $message_to_customer, $headers, $attachments );
				do_action( 'tcp_send_order_mail_to_customer', $to, $subject, $message_to_customer, $headers, $order_id );
			}

			if ( $for_merchant ) {
				$to = $thecartpress->get_setting( 'emails', false );
				if ( $to ) {
