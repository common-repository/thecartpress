...
class TCPShippingBox extends TCPCheckoutBox {
			...
			foreach( $fields as $id => $field ) {
				...
				if ( $active ) {
					...
					if ( $required && ( ! isset( $_REQUEST[$id] ) || strlen( $_REQUEST[$id] ) == 0 ) ) {
						...
						// if not billing region id, take a look to region textbox
						if ( $id == 'shipping_region_id' ) {
	...
	function showRegion( $id, $field ) {
		...
		<?php // Format: array( 'id' => array( 'name'), 'id' => array( 'name'), ... )
		$regions = apply_filters( 'tcp_load_regions_for_shipping', false ); ?>
		...
		<input type="text" id="shipping_region" name="shipping_region" value="<?php echo $region; ?>" size="20" maxlength="255" <?php if ( is_array( $regions ) && count( $regions ) > 0 ) echo 'style="display:none;"';?> class="form-control" />
