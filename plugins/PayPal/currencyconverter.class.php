...
?>
	...
	function get_conversion_rate_from_google( $data ) {
		...
		if ( false === ( $conversion_rate = get_transient( $hash ) ) ) {
			...
			$amount	= 1;
			$url	= 'http://www.google.com/finance/converter?a=' . $amount . '&from=' . $from . '&to=' . $convert_to ;
			$res	= file_get_contents( $url );

			$doc = new DOMDocument;
			@$doc->loadHTML( $res );
			$xpath = new DOMXpath( $doc );
			$conversion_rate = $xpath->query( '//*[@id="currency_converter_result"]/span' )->item(0)->nodeValue;
